...
int main()
{
    ...
    // ---------- 矩阵运算 ----------
    // 目前支持类似matlab的 .* ./ .+ .- 运算，且带有维度广播功能

    using toBroadcast1 = dim<2, 3>;
    using toBroadcast2 = dim<2, 1, 3>;

    Qu<toBroadcast1, type1> mat3 = {1, 2, 3, 4, 5, 6};
    Qu<toBroadcast2, type1> mat4 = {1, 2, 3, 4, 5, 6};

    // 输出结果的尺寸为{2, 3, 3}
    auto mat5 = mat3 + mat4;
    auto mat6 = Qmul<type1>(mat3, mat4);

    //  Qreduce 用于计算向量的和。本质是树形加法器，支持指定每层的量化模式

    Qu<dim<5>, type1> vec5 = {1, 2, 3, 4, 5};

    // 默认模式 
    auto reduce1 = Qreduce(vec5);

    // 全局指定量化，整数位宽=2 其他自动
    auto reduce2 = Qreduce<intBits<2>>(vec5);

    // 每层指定量化，注意自行确保类型数量和层数一致
    using typeList2 = TypeList<type1, type2, type1>;
    auto reduce3 = Qreduce<typeList2>(vec5);
}